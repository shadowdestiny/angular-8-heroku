...
/* harmony import */ var ngx_webstorage__WEBPACK_IMPORTED_MODULE_13__ = __webpack_require__(/*! ngx-webstorage */ "./node_modules/ngx-webstorage/fesm2015/ngx-webstorage.js");
...
let AuthHomeComponent = class AuthHomeComponent {
    constructor(projectsService) {
        this.projectsService = projectsService;
    }
    ngOnInit() {
        this.projectsService.getProjectsByUsingToken().subscribe(projects => this.projects = projects, error => console.error(error));
    }
};
...
let HeaderComponent = class HeaderComponent {
    constructor(afterLoginActionsService, toggleModalService, authService, router, localeId) {
    ...
    ngOnInit() {
        this.afterLoginActionsService.onLoginCompleted.subscribe((message) => {
            this.toggleModal();
        });
        this.toggleModalService.onToggle.subscribe((message) => {
            this.toggleModal();
        });
    }
    toggleModal() {
...
let InjectRequestsHeadersInterceptor = class InjectRequestsHeadersInterceptor {
    ...
    intercept(request, next) {
        ...
        if (token) {
            request = request.clone({
                setHeaders: Object.assign({}, headers, { Authorization: `Bearer ${token}` }),
            });
        }
        else {
            request = request.clone({
...
let FilterByPipe = class FilterByPipe {
    transform(items, searchText, fieldsToFilterBy) {
        if (!items) {
            return [];
        }
        if (!searchText) {
    ...
    prepareFiltersObject(fieldsToFilterBy, searchText) {
        const filters = {};
...
let GenerateImageUrlPipe = class GenerateImageUrlPipe {
...
let GetFieldByLocalePipe = class GetFieldByLocalePipe {
    ...
    transform(value, args) {
        if (value[args]) {
            ...
            const localizedTranslation = this.getTranslationByLocale(value.translations, locale);
            return localizedTranslation[args];
        }
        return null;
    }
    getTranslationByLocale(translations, locale) {
        const localizedTranslationArray = translations.filter(translation => translation.locale === locale);
...
let GetFirstCharPipe = class GetFirstCharPipe {
    transform(value, args) {
...
let AfterLoginActionsService = class AfterLoginActionsService {
    constructor() {
...
let AuthenticationService = class AuthenticationService {
    ...
    login(email, password) {
        const url = `${_config_api__WEBPACK_IMPORTED_MODULE_2__["API"].AUTH_SERVICES_BASE_URL}/users/login`;
...
let ToggleModalService = class ToggleModalService {
    constructor() {
...
let BookingComponent = class BookingComponent {
    ...
    filter() {
        this.bookingService.getBookings(this.init, this.end, this.id, this.firstName).subscribe((vehicles) => {
...
let BookingService = class BookingService {
    ...
    getBookings(init, end, id, firstName) {
        const serviceURL = `${_config_api__WEBPACK_IMPORTED_MODULE_4__["API"].DATA_SERVICES_BASE_URL}/bookings`;
    ...
    getBooking(slug) {
        const serviceURL = `${_config_api__WEBPACK_IMPORTED_MODULE_4__["API"].DATA_SERVICES_BASE_URL}/bookings/${slug}`;
        return this.http.get(serviceURL);
    }
    getBookingsByUsingToken() {
        const serviceURL = `${_config_api__WEBPACK_IMPORTED_MODULE_4__["API"].DATA_SERVICES_BASE_URL}/bookings/user`;
        // const token = this.getToken();
        //
        // if (token) {
...
let FullBannerComponent = class FullBannerComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let LoginFormComponent = class LoginFormComponent {
    constructor(authService, sessionStorage, afterLoginActionsService, router) {
        this.authService = authService;
        this.sessionStorage = sessionStorage;
        this.afterLoginActionsService = afterLoginActionsService;
        this.router = router;
...
let ProjectDetailComponent = class ProjectDetailComponent {
    constructor(route, projectsService, location) {
        this.route = route;
        this.projectsService = projectsService;
    ...
    getProject() {
        ...
        this.projectsService.getProject(slug).
            subscribe(project => this.project = project, error => console.error(error));
...
let ProjectsComponent = class ProjectsComponent {
    ...
    ngOnInit() {
        this.projectsService.getProjects().subscribe(vehicles => this.projects = vehicles, error => console.error(error));
    }
};
...
let ProjectsService = class ProjectsService {
    ...
    getProjects() {
        const serviceURL = `${_config_api__WEBPACK_IMPORTED_MODULE_4__["API"].DATA_SERVICES_BASE_URL}/projects`;
        return this.http.get(serviceURL);
    }
    getProject(slug) {
        const serviceURL = `${_config_api__WEBPACK_IMPORTED_MODULE_4__["API"].DATA_SERVICES_BASE_URL}/projects/${slug}`;
        return this.http.get(serviceURL);
    }
    getProjectsByUsingToken() {
        const serviceURL = `${_config_api__WEBPACK_IMPORTED_MODULE_4__["API"].DATA_SERVICES_BASE_URL}/projects/user`;
        // const token = this.getToken();
        //
        // if (token) {
        //   const headers = this.createHeadersObject(token);
...
let SingleProjectComponent = class SingleProjectComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let TimeService = class TimeService {
    ...
    getTime() {
        const serviceURL = `${_config_api__WEBPACK_IMPORTED_MODULE_3__["API"].DATA_SERVICES_BASE_URL}/time`;
        return this.http.get(serviceURL);
    }
};
...
let TimeComponent = class TimeComponent {
    ...
    ngOnInit() {
        this.timeService.getTime().subscribe(vehicles => this.times = vehicles, error => console.error(error));
    }
};
